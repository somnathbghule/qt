...
static const char USER_AGENT[] = "LoTo/" QBT_VERSION_2;
    ...
    if (m_listenInterfaceChanged) {
        ...
        for (QString ip : getListeningIPs()) {
            ...
            if (ip.isEmpty()) {
                ...
                logger->addMessage(tr("LoTo is trying to listen on any interface port: %1"
                                      , "e.g: LoTo is trying to listen on any interface port: TCP/6881")
            ...
            if (!ec) {
                ...
                logger->addMessage(tr("LoTo is trying to listen on interface %1 port: %2"
                                      , "e.g: LoTo is trying to listen on interface 192.168.0.1 port: TCP/6881")
                                   .arg(ip).arg(port)
                                   , Log::INFO);
                settingsPack.set_str(libt::settings_pack::listen_interfaces, interfacesStr);
#ifdef Q_OS_WIN
...
const QStringList Session::getListeningIPs()
{
    ...
    if (IPs.size() == 0) {
        logger->addMessage(tr("LoTo didn't find an %1 local address to listen on", "LoTo didn't find an IPv4 local address to listen on").arg(listenIPv6 ? "IPv6" : "IPv4"), Log::CRITICAL);
        IPs.append("127.0.0.1"); // Force listening to localhost and avoid accidental connection that will expose user data.
        return IPs;
    }
...
void Session::configureListeningInterface()
{
    ...
    for (const QString ip : IPs) {
        if (ip.isEmpty()) {
            logger->addMessage(tr("LoTo is trying to listen on any interface port: %1", "e.g: LoTo is trying to listen on any interface port: TCP/6881").arg(QString::number(port)), Log::INFO);
            m_nativeSession->listen_on(ports, ec, 0, libt::session::listen_no_system_port);

            if (ec)
                logger->addMessage(tr("LoTo failed to listen on any interface port: %1. Reason: %2.", "e.g: LoTo failed to listen on any interface port: TCP/6881. Reason: no such interface" ).arg(QString::number(port)).arg(QString::fromLocal8Bit(ec.message().c_str())), Log::CRITICAL);
        ...
        if (!ec) {
            logger->addMessage(tr("LoTo is trying to listen on interface %1 port: %2", "e.g: LoTo is trying to listen on interface 192.168.0.1 port: TCP/6881").arg(ip).arg(port), Log::INFO);
            return;
        }
    }
#else
...
void Session::handleListenSucceededAlert(libt::listen_succeeded_alert *p)
{
    ...
    Logger::instance()->addMessage(
        tr("LoTo is successfully listening on interface %1 port: %2/%3", "e.g: LoTo is successfully listening on interface 192.168.0.1 port: TCP/6881")
...
void Session::handleListenFailedAlert(libt::listen_failed_alert *p)
{
    ...
    Logger::instance()->addMessage(
        tr("LoTo failed listening on interface %1 port: %2/%3. Reason: %4.",
            "e.g: LoTo failed listening on interface 192.168.0.1 port: TCP/6881. Reason: already in use.")
