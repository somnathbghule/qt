...
#endif
{
    ...
    Logger::instance()->addMessage(tr("LoTo %1 started", "LoTo v3.2.0alpha started").arg(QBT_VERSION));
}
...
void Application::sendNotificationEmail(const BitTorrent::TorrentHandle *torrent)
{
    ...
    smtp->sendMail(pref->getMailNotificationSender(),
                     ...
                     tr("[LoTo] '%1' has finished downloading").arg(torrent->name()),
                     content);
}
...
int Application::exec(const QStringList &params)
{
    ...
    const QString mesg = QString("\n******** %1 ********\n").arg(tr("Information"))
        + tr("To control LoTo, access the Web UI at %1")
